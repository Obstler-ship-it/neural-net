...
class LossFunction{
    public:
        ...
        virtual Matrix<Layout::ColumnMajor> backward(const Matrix<Layout::ColumnMajor>& y_hat, const std::vector<int>& y) const{

            if (y_hat.columns != y.size())
                throw std::invalid_argument("Batch und Anzahl Labels haben unterschiedliche Größe!");

            Matrix<Layout::ColumnMajor> dL(1, y_hat.columns, Initialization::Uninitialized);
